...
jedoch bei Theorien wie der Quantenchromodynamik (QCD), so dass nur noch
...
von \citeauthor{Creutz/Statistical_Approach_QM} nachvollziehen und anwenden. Da
...
% http://www.duden.de/sprachwissen/rechtschreibregeln/namen#K134
...
\bracket{T} = 0$ \parencite[551]{Kuypers/Mechanik}. $\vec F \vec r$ kann auch
als $- [\vnabla V] \vec r$ geschrieben werden. In einer Dimension reduziert
sich dies auf
\[
...
% XXX Dieser Absatz ist nicht sonderlich wichtig.
...
Gleichung~\eqref{eq:entwicklung} setze ich (analog zu \br5) in die
...
\[
    V(x) = \frac12 \mu^2 x^2 - \frac{2}{a_0} \frac1{\sqrt{2\piup}\sigma} \exp\del{-\frac{x^2}{\sigma^2}}.
\]
Somit sagt der Virialsatz aus:
\[
    \bracket T = \frac12 \mu^2 \bracket{x^2} + \frac{2}{a_0} \frac1{\sqrt{2\piup}\sigma^3} \Bracket{\exp\del{-\frac{x^2}{\sigma^2}} x^2}.
\]
Zusammen mit dem zeitlichen Mittelwert der potentiellen Energie gibt dies die
Gesamtenergie:
\[
    \bracket E = \mu^2 \bracket{x^2} + \frac{2}{a_0} \frac1{\sqrt{2\piup}\sigma} \sbr{\frac1{\sigma^2} \Bracket{\exp\del{-\frac{x^2}{\sigma^2}} x^2} - \Bracket{\exp\del{-\frac{x^2}{\sigma^2}}}}.
\]

\chapter{Ergebnisse}

Die in den vorhergehenden Kapiteln beschrieben Methoden habe ich in einem C++
Programm umgesetzt. Mein Programm ist freie Software und kann auf meiner
Webseite \url{http://martin-ueding.de/de/university/physik590/} heruntergeladen
werden.

Die Weltlinie wird in der Regel anfangs mit zufälligen Werten für die $x_j$ aus
dem Intervall $(-\initialrandomwidth, \initialrandomwidth)$ initialisiert.
Bevor Werte aufgenommen werden, durchläuft das System $\preiterations$
Monte-Carlo-Iterationen (meist \num{50}) zur Relaxation. Danach sieht die
Weltlinie wie in Abbildung~\ref{fig:relaxiert} aus.
...
\begin{figure}[htbp]
    ...
    \caption{%
        ...
        Eigenwerte. An die Daten wurde jeweils eine abfallende
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[black] table {CSV/gauss-potential/pos/18B9E2-potential.csv}; % 1.00
            \addplot[black] table {CSV/gauss-potential/pos/DF05AB-potential.csv}; % 0.800
            \addplot[black] table {CSV/gauss-potential/pos/00C58F-potential.csv}; % 0.500
            \addplot[black] table {CSV/gauss-potential/pos/53D7EB-potential.csv}; % 0.300
            \addplot[black] table {CSV/gauss-potential/pos/6FC501-potential.csv}; % 0.100
        \end{axis}
    \end{tikzpicture}
    \end{minipage}
    \hfill
...
Korrelationen, die die Energiewerte verfälschen. Dadurch steigt die benötigte
Rechenleistung an, es kann nur bis zu einem gewissen $\timestep$ gerechnet
werden. Beim Grenzwert $\gausswidth \to 0$ wird die überlagerte
Normalverteilung beliebig schmal. Ist sie zu schmal, ist die
Wahrscheinlichkeit, dass Zufallszahlen in ihrer Breite gezogen werden, zu
...
Für alle $\timestep$ ist zu erkennen, dass für kleine $\gausswidth$ die
Grundzustandsenergie $E_0$ gegen die $\num{0.5}$ des harmonischen Oszillators
geht. Es ist jedoch verwunderlich, dass die Energiewerte, die dem
Kontinuumswert von \num{1.08} am nächsten kommen, bei $\gausswidth \approx
\num{0.3}$ liegen. Der Kontinuumswert sollte bei $\gausswidth \to 0$ erreicht
werden. Außerdem sinkt die Grundzustandsenergie mit kleinem $\timestep$
ebenfalls. Diesen Effekt konnte ich auch für den harmonischen Oszillator
beobachten, es ist also unabhängig von der Störung. Der Grenzwert $\timestep
\to 0$ sollte auf den Kontinuumswert führen.
...
${E_0}_\gausswidth(\timestep)$. In Abbildung~\ref{fig:minus1-fein} sind die
Scharen dargestellt.
...
\begin{figure}[htbp]
    ...
    \caption{%
        ...
        Anpassungsfunktionen. Es sind zur Übersicht nur die kleinen Werte von
        $\gausswidth$ dargestellt.
    }
    \label{fig:minus1-fein}
\end{figure}

Für alle Werte von $\gausswidth$ ist der Abfall bei kleinen $\timestep$ zu
...
vorherigen Daten zur negativen Kopplungsstärke nähert sich hier bei sehr
kleinen $\timestep$ der Energiewert an den theoretischen Wert an. Alle
Datenpunkte sind in Abbildung Abbildung~\ref{fig:plus1-E0-a} auf der linken
Seite gezeigt. Rechts ist eine Vergrößerung des interessanten Energiebereichs
um $E_0 = \num{-1.94107}$ dargestellt. Für $\gausswidth = \num{0.01}$ und
$\timestep = \num{0.005}$ ist dieser Kontinuumswert fast erreicht.
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[
                ...
                y dir=both,
                y explicit
            ] table[y error index=2] {CSV/+1/sigma-E0-0.005.csv};
...
Auf die gleiche Weise sollten auch die angeregten Zustände für $1/a_0 = -1$ zu
bestimmen sein. Jedoch streuen die Daten zu sehr, als dass der Unterschied
zwischen \num{1.85} und \num{2} für den gestörten bzw. harmonischen Oszillator
sichtbar wäre. Die Daten sind in Abbildung~\ref{fig:mesh-e2-e0} dargestellt.
...
\begin{figure}[htbp]
    ...
    \caption{%
        ...
        Normalverteilung für eine Kopplungsstärke von $1/a_0 = -1$.
    }
...
% TODO Zusammenfassung erstellen.
