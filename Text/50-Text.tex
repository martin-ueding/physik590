...
numerische Methoden notwendig sind \parencite{Wittig/Farbdynamik_Gitter}.
...
ist unbeschränkt. Aufgrund der hohen Dimension sind Monte-Carlo-Methoden zu
...
Die Übergangswahrscheinlichkeit mit Zwischenposition $x_j$ wird durch
Integration über diese ermittelt
\parencite[(3.9)]{Creutz/Statistical_Approach_QM}:
\[
    W^{(2)}\del{\vec x^{(i)}, \vec x^{(k)}} = \int \dif x^{(j)} \, W\del{\vec
    x^{(i)}, \vec x^{(j)}} W\del{\vec x^{(j)}, \vec x^{(k)}}.
\]
Daraus kann die Rekursionsformel
\[
...
Es werden also neue Werte für $x_j$, nämlich $x_j'$, aus einer Verteilung
...
Die Wirkungsänderung $\Deltaup S$ beim Übergang von $x_j$ zu $x_j'$ wird
effizient dadurch berechnet, dass nur die Wirkung der Schritte von $j-1$ zu
$j$ und von $j$ zu $j+1$ berechnet wird. Dazu definiere ich die Wirkung für
einen Gitterschritt:
\[
...
Damit kann ich die Wirkungsänderung zwischen zwei Weltlinien, die sich in nur
...
Für das GEVP werden Interpolationsfelder $O_i$ gewählt
...
Das generalisierte Eigenwertproblem, aus dessen Eigenwerten die Energien
extrahiert werden können, ist durch
\[
...
\begin{figure}[htbp]
    \centering
    \tikzsetnextfilename{plus1-E0-E2-a-zoom}
    \begin{tikzpicture}
        \begin{axis}[
                width=\linewidth,
                height=0.5\linewidth,
                xlabel={$a$},
                ylabel={$\Deltaup E_{2,0} = E_2 - E_0$},
                xmax=0.02,
                %yticklabel pos=right,
                grid=major,
                myerrorbar/.style={
            ...
            \addplot[
                ...
                myerrorbar,
                mark=triangle,
            ] table[y error index=2] {CSV/+1/a-E2-E0-0.3.csv};
            \addplot[
                black!100!white,
                myerrorbar,
                mark=*,
            ] table[y error index=2] {CSV/+1/a-E2-E0-0.2.csv};
            \addplot[
                black!100!white,
                myerrorbar,
                mark=o,
            ] table[y error index=2] {CSV/+1/a-E2-E0-0.1.csv};
            \addplot[
                black!100!white,
                myerrorbar,
                mark=square*,
            ] table[y error index=2] {CSV/+1/a-E2-E0-0.05.csv};
            \addplot[
                black!100!white,
                myerrorbar,
                mark=square,
            ] table[y error index=2] {CSV/+1/a-E2-E0-0.01.csv};
    ...
    \end{tikzpicture}
    \caption{%
        Differenz von $E_2$ und $E_0$, die mittels GEVP bestimmt worden sind.
        Der Stern markiert den Kontinuumswert.
    }
    \label{fig:plus1-E0-E2-a-zoom}
\end{figure}

\chapter{Zusammenfassung}

In dieser Arbeit habe ich die harmonischen Oszillator, sowie einen Oszillator
mit $\delta$-Störung, in der Pfadintegralformulierung untersucht. Dazu habe ich
imaginäre und diskrete Zeit benutzt. Aus den mit dem
\textsc{Metropolis}algorithmus generierten Daten bestimmte ich mit Virialsatz
und Korrelationen die Energiewerte.
