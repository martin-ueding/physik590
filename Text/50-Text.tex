...
können. Bei einigen von ihnen lässt sich das Problem jedoch in ein analytisch
lösbaren Teil und einen Störteil aufteilen. Die Störungstheorie versagt jedoch
bei Theorien wie der Quantenchromodynamik, so dass nur noch numerische Methoden
anwendbar sind \parencite{Wittig/Farbdynamik_Gitter}.

Anhand des harmonischen Oszillators, der analytisch gelöst werden kann, möchte
ich in dieser Arbeit die Methoden von
\citeauthor{Creutz/Statistical_Approach_QM} nachvollziehen und anwenden. Auf
diese Weise können die numerischen Ergebnisse mit den Analytischen verglichen
werden.
...
werden. Diese werden als gegeben vorausgesetzt und nicht mehr unbedingt
explizit zitiert.
...
bestimmt werden. Dieser besagt allgemein
$\bracket{\vec F \vec r} + 2 \bracket{T} = 0$ \parencite[551]{Kuypers/Mechanik}. $\vec F \vec r$ kann auch als $-
[\vnabla V] \vec r$ geschrieben werden. In einer Dimension
ist das, wie in \parencite[(2.29)]{Creutz/Statistical_Approach_QM},
\[
...
Für alle $\timestep$ ist zu erkennen, dass für große und kleine $\gausswidth$
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        \begin{axis}[
                ...
                myerrorbar/.style={
                    ...
                    %error bars/error mark options=solid,
                },
                myfit/.style={
                    solid,
                },
    ...
    \caption{%
        ...
        Anpassungsfunktionen. Es sind nur die großen Werte von $\gausswidth$
        dargestellt.
    }
    \label{fig:minus1-grob}
\end{figure}

Für alle Werte von $\gausswidth$ ist der starke Abfall bei kleinen $\timestep$
zu erkennen. An die Werte mit $\timestep \geq \num{0.05}$ habe ich eine
Funktion zweiten Grades angepasst, um bis $\timestep = 0$ extrapolieren zu
können. Je schmaler die Normalverteilung wird, desto größer wird die
Grundzustandsenergie. Dies geht allerdings nur bis $\gausswidth = \num{0.4}$,
danach sinken die Energien, zumindest für große $\timestep$, wieder.

Die zweite Hälfte der Daten ist als Projektion in
Abbildung~\ref{fig:minus1-fein} dargestellt. Hier ist der gleiche Abfall bei
kleinen $\timestep$ zu erkennen. Bei $\timestep = \num{0.05}$ ist die Energie
wieder am höchsten, bei größeren $\timestep$ jedoch deutlich kleiner. Auch an
diese Daten wurden Funktionen zweiten Gerades angepasst.
...
\begin{appendix}
    ...
    % TODO Dies reicht für ein einzelnes Kapitel nicht aus, jedoch passt es
    % auch irgendwie noch an keinen Ort so richtig.
