...
\]
Dieser Integrand wird nur in der Nähe der minimalen Wirkung Beiträge liefern.
...
$\timesites+1$ ($0, \ldots, \timesites$)
Zeitpunkte zu:
\[
    \int [\dif x] \leadsto \prod_{j = 1}^{\timesites-1} \int_{-\infty}^\infty \dif x_j.
\]
...
beliebe Wirkung in die Zustandssumme einfließen können, fließen mit imaginärer
Zeit Weltlinien mit großer Geschwindigkeit (und daher großer Wirkung) deutlich
weniger stark ein. Daher ist die Benutzung eines Zeitgitters in Kombination mit
imaginärer Zeit legitim. Im Grenzfall $\timestep \to 0$ erhält man den
kontinuierlichen Fall zurück.
...
$\iterations \to \infty$ nähert sich die Verteilung der Benötigten an.
...
\parencite[(2.1)]{Blossier/Eigenvalue}
\[
    C_{ij} := \Bracket{O_i(t) \, O_j^*(t)}
\]

\subsection{Lösung durch Cholesky-Zerlegung}
...
$\tens L$, so dass $\tens B = \tens L \tens L^\dagger$ gilt.
...
\texttt{Eigen::MatrixBase::llt()} aus der C++ Bibliothek Eigen
\parencite{Eigen/LLT}. Das normale Eigenwertproblem löse ich anschließend mit
der Klasse \texttt{Eigen::EigenSolver} \parencite{Eigen/EigenSolver}.
