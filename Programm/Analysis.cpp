...
Analysis::Analysis(BootstrapPool &pool, Settings &settings)
    : pool{pool},
      settings{settings},
      boot_hist{BootstrappedHistogram{settings.position_hist_min,
    ...
    for (unsigned t_id = 0; t_id != settings.correlation_ts.size(); t_id++) {
        ...
        for (unsigned n{0}; n < settings.max_energyvalue(); n++) {
            inner.emplace(std::piecewise_construct,
                          std::make_tuple(n),
                          std::make_tuple());
        }
...
void
Analysis::insert_eigenvalues(CorrList &C, bool even, BQMapMap &bs_lambda_n_t) {
    for (unsigned t_id = 0; t_id != settings.correlation_ts.size(); t_id++) {
        ...
        for (unsigned i{cutoff}; i < lambda_i_t.size(); i++) {
            ...
            bs_lambda_n_t[t_id][settings.matrix_to_state(i - cutoff, even)]
                .append(lambda);
        }
    }
}
...
void Analysis::create_samples() {
    unsigned largest = *std::max_element(settings.correlation_ts.begin(),
                                         settings.correlation_ts.end());
...
void Analysis::worker(ProgressBar &bar) {
    ...
    while ((sample_id = sample_id_atom++) < settings.bootstrap_samples) {
        ...
        for (unsigned t_id = 0; t_id != settings.correlation_ts.size();
             t_id++) {
            unsigned t{settings.correlation_ts[t_id]};
            Eigen::MatrixXd &c_t{odd[t_id]};
            double c_t_11{
                c_t(settings.state_to_matrix(1), settings.state_to_matrix(1))};
            c11[t].append(c_t_11);
        }
...
void Analysis::save_eigenvalues() {
    ...
    for (unsigned n{1}; n <= settings.max_energyvalue(); n++) {
        ...
        filename << "eigenvalue-" << std::setfill('0') << std::setw(2) << n
                 << ".csv";
        for (unsigned t_id = 0; t_id != settings.correlation_ts.size();
             t_id++) {
            try {
                ...
                output << settings.correlation_ts[t_id] * settings.time_step
...
void Analysis::save_c11() {
    ...
    for (unsigned i : settings.correlation_ts) {
        try {
            c11_handle << i *settings.time_step << "\t" << c11[i].mean() << "\t"
                       << c11[i].stddev() << std::endl;
        } catch (std::runtime_error e) {
...
void Analysis::save_histogram() {
    boot_hist.write_histogram(
        settings.generate_filename("position-density.csv"));
}
