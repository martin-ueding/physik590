...
Oscillator::Oscillator(Settings &settings) : settings{settings} {
    ...
    prefactor = 1. / std::sqrt(2. * pi * gauss_width_squared) * 2. *
...
double Oscillator::action_difference(double prev,
                                     double cur,
                                     double alt,
                                     double next) {
...
double Oscillator::action_step(double cur, double next) {
    ...
    double kinetic_part{0.5 * settings.mass * (difference * difference) /
                        (settings.time_step * settings.time_step)};
    double potential_part{potential(cur)};
    return settings.time_step * (kinetic_part + potential_part);
}

void Oscillator::export_potential(std::string filename,
                                  std::string preamble,
                                  Settings &settings) {
    std::ofstream handle{filename};
    handle << preamble;
    double step{settings.export_potential_bound / 2 /
                settings.export_potential_steps};
    for (double x{-settings.export_potential_bound};
         x <= settings.export_potential_bound;
         x += step) {
