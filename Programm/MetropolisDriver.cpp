...
void MetropolisDriver::run(ResultSet &results) {
    ...
    trajectory.save_plot_file(settings.generate_filename(
                                  "out/trajectory-05-end-", ".csv"));
    position_histogram.save(settings.generate_filename(
                                "out/histogram-position-", ".csv"));

    action_list.binning_snapshot(action_histogram);
    action_list.save_plot_file(settings.generate_filename(
                                   "out/trajectory-action-", ".csv"));
    action_histogram.save(settings.generate_filename(
                              "out/histogram-action-", ".csv"));

    std::cout << "Accept Rate Total:       " << ma.get_accept_rate()
              << std::endl;
