...
MetropolisDriver::MetropolisDriver(Settings settings)
    : ma{MetropolisAlgorithm{
          system, settings.position_seed, settings.accept_seed}},
      x{std::vector<double>(settings.time_sites)},
      settings{settings},
      system{Oscillator{settings}} {
    system.export_potential(settings.generate_filename("potential.csv"),
                            settings.report(),
                            settings);
    ...
    vh.save_plot_file(x,
                      settings.generate_filename("trajectory-01-init.csv"),
                      settings.report());

    vh.set_to_random(x, settings.initial_random_width);
    vh.save_plot_file(x,
                      settings.generate_filename("trajectory-02-random.csv"),
                      settings.report());

    for (unsigned i = 0;
         i < settings.pre_iterations - settings.iterations_between;
         i++) {
        ma.iteration(x, settings.pre_rounds, settings.margin);
    }
    vh.save_plot_file(
        x,
        settings.generate_filename("trajectory-04-more_iterations.csv"),
        settings.report());
}
